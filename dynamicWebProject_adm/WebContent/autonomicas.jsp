...
<body>
	...
	int cod_campanna = 0;
		...
		ArrayList<Programa> programaAutonomico = controladorBD.dameProgramasPorCod_Campanna(cod_campanna);
		ArrayList<Candidato> candidatosAutonomico = controladorBD.dameCandidatosCampanna(cod_campanna);
			...
			<div class="row">
				...
				<div id="content" class="8u skel-cell-important">
					<section>
						...
						<%
						if(cod_campanna!=0){
								for (int i = 0; i < programaAutonomico.size(); i++) {

									String puntoCorto = programaAutonomico.get(i).getDescripcion().substring(0, 30) + "...";
							%>
							<li id="punto<%=programaAutonomico.get(i).getCod_punto()%>"
								onclick="mostrar(this.id)" class="pulsable"><%=(i + 1) + ". " + puntoCorto%></li>
							<li id="punto<%=programaAutonomico.get(i).getCod_punto()%>completo"
								class="pulsable ocultar"><%=(i + 1) + ". " + programaAutonomico.get(i).getDescripcion()%></li>
							<%
								}
								}
							%>
				...
				<div id="sidebar" class="4u">
					<section>
						...
						<ul class="default">
							...
							<%
							if(cod_campanna!=0){
								for (int i = 0; i < candidatosAutonomico.size(); i++) {
									if (i == 0) {
							%>
							<li id="<%=candidatosAutonomico.get(i).getCod_candidato()%>" onclick="info(this.id)" style="color: green;" class="pulsable"><%=(i + 1) + ". " + posicionCandidato.get(i + 1)%></li>
							<%
								} else {
							%>
							<li id="<%=candidatosAutonomico.get(i).getCod_candidato()%>" onclick="info(this.id)" class="pulsable"><%=(i + 1) + ". " + posicionCandidato.get(i + 1)%></li>
							<%
								}
							%>
						...
						<div id="modal" class="modal">
							...
							<div class="modal-content ">
								...
								%>
								<div
									id="candidatoModal<%=candidatosAutonomico.get(i).getCod_candidato()%>" class="ocultar">
									<img class="foto"
										src="images/candidatos/<%=candidatosAutonomico.get(i).getCod_candidato()%>.jpg" />
									<div class="info">
										<h2><%=candidatosAutonomico.get(i).getNombre() + " " + candidatosAutonomico.get(i).getApellidos()%></h2>
										<p><%=candidatosAutonomico.get(i).getFecha_nac()%></p>
										<p><%=candidatosAutonomico.get(i).getMunicipio()%></p>
									</div>
								</div>
								<%
									}
