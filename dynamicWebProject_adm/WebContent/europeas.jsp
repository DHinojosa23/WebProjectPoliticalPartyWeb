...
<body>
	...
	<%
		...
		ArrayList<Programa> programaEuropa = controladorBD.dameProgramasPorCod_Campanna(1);
		ArrayList<Candidato> candidatosEuropa = controladorBD.dameCandidatosCampanna(1);

		Hashtable<Integer, String> posicionCandidato = new Hashtable<Integer, String>();
		posicionCandidato = controladorBD.dameNombreCandidatoyPosicionHashPorCampanna(1);
		...
		<div class="container">
			<div class="row">
				...
				<div id="content" class="8u skel-cell-important">
					<section>
						<header class="major">
							<h2>PROGRAMA EUROPEO</h2>
							<span class="byline">Propuestas Electorales 2019</span>
						</header>
						<p>Por la Europa de las personas: Reinicia Europa.</p>
						<br>
						<p>Puntos a destacar:</p>
						<br>
						<ol>
							<%
								for (int i = 0; i < programaEuropa.size(); i++) {

									String puntoCorto = programaEuropa.get(i).getDescripcion().substring(0, 30) + "...";
							%>
							<li id="punto<%=programaEuropa.get(i).getCod_punto()%>"
								onclick="mostrar(this.id)" class="pulsable"><%=(i + 1) + ". " + puntoCorto%></li>
							<li id="punto<%=programaEuropa.get(i).getCod_punto()%>completo"
								class="pulsable ocultar"><%=(i + 1) + ". " + programaEuropa.get(i).getDescripcion()%></li>
							<%
								}
							%>
				...
				<div id="sidebar" class="4u">
					<section>
						...
						<ul class="default">
							...
							<%
								for (int i = 0; i < candidatosEuropa.size(); i++) {
									if (i == 0) {
							%>
							<li id="<%=candidatosEuropa.get(i).getCod_candidato()%>" class="pulsable" onclick="info(this.id)" style="color: green;"><%=(i + 1) + ". " + posicionCandidato.get(i + 1)%></li>
							<%
								} else {
							%>
							<li id="<%=candidatosEuropa.get(i).getCod_candidato()%>" onclick="info(this.id)" class="pulsable"><%=(i + 1) + ". " + posicionCandidato.get(i + 1)%></li>
							<%
								}
							%>
						...
						<div id="modal" class="modal">
							...
							<div class="modal-content ">
								...
								%>
								<div
									id="candidatoModal<%=candidatosEuropa.get(i).getCod_candidato()%>" class="ocultar">
									<img class="foto"
										src="images/candidatos/<%=candidatosEuropa.get(i).getCod_candidato()%>.jpg" />
									<div class="info">
										<h2><%=candidatosEuropa.get(i).getNombre() + " " + candidatosEuropa.get(i).getApellidos()%></h2>
										<p><%=candidatosEuropa.get(i).getFecha_nac()%></p>
										<p><%=candidatosEuropa.get(i).getMunicipio()%></p>
									</div>
								</div>
								<%
									}
